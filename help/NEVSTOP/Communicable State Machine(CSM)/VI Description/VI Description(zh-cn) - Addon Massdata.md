...
将 massdata 参数转换为原始数据。

> 参考: CSM Massdata 参数格式

-- <b>输入控件 (Controls)</b> --
- <b>MassData Argument</b>: 输入的 massdata 参数
- <b>Type</b>: 数据类型
...
将数据转换为 massdata 参数，此转换得到的 massdata 参数不包含数据类型信息。
...
-- <b>输出控件 (Indicators)</b> --
- <b>MassData Argument</b>: 转换后的 massdata 参数

## CSM - Convert MassData to Argument With DataType.vim

将数据转换为 massdata 参数，此转换得到的 massdata 参数包含数据类型信息。
...
辅助 VI，用于根据缓存状态更新界面指定的 massdata 缓存显示控件，可更加直观地展示缓存的使用情况。

-- <b>输入控件 (Controls)</b> --
- <b>MassData Status Indicator</b>: massdata 缓存显示控件引用
